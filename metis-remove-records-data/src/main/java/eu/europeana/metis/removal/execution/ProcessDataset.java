package eu.europeana.metis.removal.execution;

import static eu.europeana.metis.removal.config.PropertiesHolder.STATISTICS_LOGS_MARKER;
...
import eu.europeana.metis.removal.model.FailedRecord;
...
public class ProcessDataset implements Callable<Void> {
  ...
  ProcessDataset(DatasetStatus datasetStatus, Configuration configuration,
  ...
  private void processDataset() throws ExecutionException, InterruptedException {
    ...
    if (configuration.getMode() == Mode.DEFAULT || configuration.getMode() == Mode.DRY_RUN) {
  ...
  private void finalizeDatasetStatus(long startProcess) {
    ...
    datasetStatus.setActualTimeProcessAndIndex(datasetStatus.getActualTimeProcessAndIndex() + elapsedTime);
    configuration.getMongoSourceMongoDao().storeDatasetStatusToDb(datasetStatus);
  ...
  private void failedRecordsOperation() {
    ...
    while (CollectionUtils.isNotEmpty(nextPageOfRecords)) {
      ...
      for (FullBeanImpl fullBean : nextPageOfRecords) {
        final String exceptionStackTrace = process(fullBean);
  ...
  private String process(FullBeanImpl fullBean) {
    try {
      final RDF rdf = processFullBeanRecord(fullBean);
      processIndexedRecord(rdf, fullBean);
      ...
      LOGGER.error("{} - Could not process or depublish(RuntimeException) record: {}", prefixDatasetIdLog, fullBean.getAbout(),
          e);
      return exceptionStacktraceToString(e);
    }
    return "";
  ...
  private void processIndexedRecord(RDF rdf, FullBeanImpl fullBean) throws IndexingException {
      ...
      if (StringUtils.isNotBlank(resourceId)) {
        if (StringUtils.isBlank(exceptionStackTrace)) {
          if (processFailedOnly) {
            configuration.getMongoSourceMongoDao()
          ...
          configuration.getMongoSourceMongoDao().storeFailedRecordToDb(failedRecord);
          if (!processFailedOnly) {
            datasetStatus.setTotalProcessed(datasetStatus.getTotalProcessed() + 1);
            datasetStatus.setTotalFailedRecords(datasetStatus.getTotalFailedRecords() + 1);
          }
  ...
  private RDF processFullBeanRecord(FullBeanImpl fullBean) throws ProcessingException {
    final long startTimeProcess = System.nanoTime();
    try {
      return configuration.getFullBeanProcessor().apply(fullBean, configuration);
    } finally {
