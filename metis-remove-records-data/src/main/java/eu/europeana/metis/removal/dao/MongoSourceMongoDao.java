...
public class MongoSourceMongoDao {
  ...
  public List<String> getAllDatasetIds() {
    List<GroupedDataset> datasets = mongoSourceDatastore
  ...
  public void dropTemporaryCollections() {
    mongoSourceDatastore.getDatabase().getCollection(DatasetStatus.class.getSimpleName())
                        .drop();
    mongoSourceDatastore.getDatabase().getCollection(FailedRecord.class.getSimpleName())
                        .drop();
  }

  /**
   * Gets all dataset statuses.
   *
   * @return the all dataset statuses
   */
  public List<DatasetStatus> getAllDatasetStatuses() {
    return MorphiaUtils
        .getListOfQueryRetryable(mongoSourceDatastore.find(DatasetStatus.class));
  }
  ...
  public DatasetStatus getDatasetStatus(String datasetId) {
    return mongoSourceDatastore.find(DatasetStatus.class)
                               .filter(Filters.eq(DATASET_ID, datasetId)).first();
  }

  /**
   * Store dataset status to db.
   *
  ...
  public void storeDatasetStatusToDb(DatasetStatus datasetStatus) {
    ExternalRequestUtil.retryableExternalRequestForNetworkExceptions(
        () -> mongoSourceDatastore.save(datasetStatus));
  }

  /**
   * Store failed record to db.
   *
  ...
  public void storeFailedRecordToDb(FailedRecord failedRecord) {
    ...
    ExternalRequestUtil.retryableExternalRequestForNetworkExceptions(
        () -> mongoSourceDatastore.save(failedRecord));
  }

  /**
   * Delete failed record from db.
   *
   * @param failedRecord the failed record
   */
  public void deleteFailedRecordFromDb(FailedRecord failedRecord) {
    //Will replace it if already existent
    ExternalRequestUtil.retryableExternalRequestForNetworkExceptions(
        () -> mongoSourceDatastore.delete(failedRecord));
  }

  /**
   * Gets next page of failed records.
   *
   * @param datasetId the dataset id
   * @param nextPage the next page
   * @return the next page of failed records
   */
  public List<FailedRecord> getNextPageOfFailedRecords(String datasetId, int nextPage) {
    Query<FailedRecord> query = mongoSourceDatastore.find(FailedRecord.class);
