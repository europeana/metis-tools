...
public class ExecutorManager implements AutoCloseable {
  ...
  public void startRecordsRemovalData() throws InterruptedException {
    LOGGER.info("Starting with mode, startIndex, endIndex, " + "totalAllowedThreads: {}, {}, {}, {}",
            ...
            totalAllowedThreads);
   if (configuration.getMode().equals(Mode.POST_CLEAN)) {
      //We only clean/remove the collections that were created for assisting the processing
      configuration.getMongoSourceMongoDao().dropTemporaryCollections();
      return;
    }
  ...
  private List<DatasetStatus> getDatasetStatuses() {
    ...
    if (configuration.getMode().equals(Mode.REPROCESS_ALL_FAILED)) {
      datasetStatuses = configuration.getMongoSourceMongoDao().getAllDatasetStatuses();
  ...
  private DatasetStatus retrieveOrInitializeDatasetStatus(String datasetId, int indexInOrderedList,
    ...
    DatasetStatus retrievedDatasetStatus = configuration.getMongoSourceMongoDao()
    ...
    if (retrievedDatasetStatus == null) {
      ...
      configuration.getMongoSourceMongoDao().storeDatasetStatusToDb(retrievedDatasetStatus);
    }
  ...
  private class ScheduledThreadForSpeedProjection extends TimerTask {
    ...
    ScheduledThreadForSpeedProjection(Date startDate, List<DatasetStatus> datasetStatuses) {
      ...
      this.totalPreviouslyProcessed = datasetStatuses
          .stream()
          .map(
              datasetStatus -> configuration.getMongoSourceMongoDao()
                                            .getDatasetStatus(datasetStatus.getDatasetId()))
          .filter(Objects::nonNull)
          .mapToLong(DatasetStatus::getTotalProcessed)
          .sum();
