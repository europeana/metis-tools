...
public class ProcessorRunner implements CommandLineRunner {
  ...
  private Map<String, Long> getDatasetWithSize(long limit) {
    ...
    return mongoCoreDao.getAllDatasetIds().parallelStream()
                       .map(datasetId -> new SimpleEntry<>(datasetId,
                           mongoSourceDao.getTotalRecordsForDataset(datasetId)))
    ...
    try {
      ...
      if (datasetPageNumber.getDatasetId() == null) {
        ...
        datasetPageNumber.setFullBeanList(
            mongoSourceDao.getNextPageOfRecords(datasetPageNumber.getDatasetId(), datasetPageNumber.getPage(),
                applicationProperties.getRecordPageSize()));
      }
