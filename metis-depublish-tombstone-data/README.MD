...
This project contains functionality to remove records from tombstone repository.
...
- `record.ids.to.process` -> A comma separated list of records ids to process from an specific dataset.
...
- `identity.process` -> Set to true to do a simple read dry run without altering the record
...
<u>*Version 1(Spring 2025)*</u>  
The goal is to remove tombstone records from a dataset with the following values GDPR, PERMISSION_ISSUES, SENSITIVE_CONTENT  
