package eu.europeana.metis.sandbox.reindexer;

import eu.europeana.metis.sandbox.reindexer.model.Context;
import eu.europeana.metis.sandbox.reindexer.model.Reindexer;
...
public class Application {
   ...
   * The Context.
   */
  static eu.europeana.metis.sandbox.reindexer.model.Context context = new Context();
  ...
  public static void main(String[] args) throws ParseException {
    ...
    eu.europeana.metis.sandbox.reindexer.model.Reindexer reindexer = new Reindexer(context);
    reindexer.run();
    LOGGER.info("Done!");
    System.exit(0);
  }
  ...
  public static void setIntegerParam(String paramName, int defaultValue) {
    ...
    context.getIntParams().put(paramName, paramValue);
  }
   ...
   * @param defaultValue the default value
   */
  public static void setStringParam(String paramName, String defaultValue) {
    String paramValue = cmd.getOptionValue(paramName);
    if (paramValue == null) {
      paramValue = defaultValue;
    }
    context.getStringParams().put(paramName, paramValue);
  }
  ...
  private static void parseCmdLine(String[] args) throws ParseException {
    ...
    options.addOption(Option.builder("queueSize")
                            .desc(
                                "Maximum size of the in-memory queue between reading and writing (number of pages). Defaults to 100")
                            .hasArg(true)
                            .build());
    ...
    options.addOption(Option.builder("sourceZkAddress")
                            .desc(
                                "Zookeeper addresses for source SolrCloud. Defaults to 'localhost:2181'. Comma-separate multiple addresses")
                            .hasArg(true)
                            .build());
    ...
    options.addOption(Option.builder("targetZkAddress")
                            .desc(
                                "Zookeeper addresses for target SolrCloud. Defaults to 'localhost:2181'. Comma-separate multiple addresses")
                            .hasArg(true)
                            .build());
