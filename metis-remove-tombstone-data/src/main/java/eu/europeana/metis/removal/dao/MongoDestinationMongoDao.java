...
public class MongoDestinationMongoDao {
  ...
  public List<FailedRecord> getNextPageOfFailedRecords(String datasetId, int nextPage) {
    Query<FailedRecord> query = mongoDestinationTombstoneDatastore.find(FailedRecord.class);
  ...
  public List<DatasetStatus> getAllDatasetStatuses() {
    return MorphiaUtils
        .getListOfQueryRetryable(mongoDestinationTombstoneDatastore.find(DatasetStatus.class));
  }

  public DatasetStatus getDatasetStatus(String datasetId) {
    return mongoDestinationTombstoneDatastore.find(DatasetStatus.class)
                                             .filter(Filters.eq(DATASET_ID, datasetId)).first();
  }

  public void deleteDatasetStatus(String datasetId) {
    mongoDestinationTombstoneDatastore.find(DatasetStatus.class).filter(Filters.eq(DATASET_ID, datasetId))
                                      .delete();
  }
  ...
  public void dropTemporaryCollections() {
    mongoDestinationTombstoneDatastore.getDatabase().getCollection(DatasetStatus.class.getSimpleName())
                                      .drop();
    mongoDestinationTombstoneDatastore.getDatabase().getCollection(FailedRecord.class.getSimpleName())
                                      .drop();
  }
  ...
  public void storeDatasetStatusToDb(DatasetStatus datasetStatus) {
    ExternalRequestUtil.retryableExternalRequestForNetworkExceptions(
        () -> mongoDestinationTombstoneDatastore.save(datasetStatus));
  }

  public void storeFailedRecordToDb(FailedRecord failedRecord) {
    //Will replace it if already existent
    ExternalRequestUtil.retryableExternalRequestForNetworkExceptions(
        () -> mongoDestinationTombstoneDatastore.save(failedRecord));
  }

  public void deleteFailedRecordFromDb(FailedRecord failedRecord) {
    //Will replace it if already existent
    ExternalRequestUtil.retryableExternalRequestForNetworkExceptions(
        () -> mongoDestinationTombstoneDatastore.delete(failedRecord));
  }
